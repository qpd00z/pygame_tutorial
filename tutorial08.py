...
robowinsound = pygame.mixer.Sound("snd/robowin.wav")
robolosesound = pygame.mixer.Sound("snd/robolose.wav")
...
def collision():
    global balls, lost, won, active
    # zombie hit box
    zombiehitbox = pygame.Rect(int(zombie1.x) + 18, int(zombie1.y) + 24, zombie1.width - 36, zombie1.height - 24)
    # player hit box
    playerhitbox = pygame.Rect(int(player1.x) + 18, int(player1.y) + 36, player1.width - 36, player1.height - 36)

    for b in balls:
        ballhitbox = pygame.Rect(int(b.x) - b.radius, int(b.y) - b.radius, b.radius * 2, b.radius * 2)
        if zombiehitbox.colliderect(ballhitbox):
            balls.remove(b)
            zombie1.lifes -= 1
            if zombie1.lifes <= 0 and not lost:
                won = True
                pygame.mixer.Sound.play(robowinsound)
                active = False

    if zombiehitbox.colliderect(playerhitbox):
        lost = True
        won = False
        pygame.mixer.Sound.play(robolosesound)
        active = False
...
player1 = Player(300, 393, 4, 96, 128, -16, [0, 0, 1, 0], 0, 0)
